 ...
 *
 * Command: mtd - List MTD (Memory Technology Device) partitions
 */
...
int cmd_mtd(conn_t *conn, uint32_t id, const uint8_t *args, size_t args_len)
{
    ...
    if (!f) {
        if (errno == ENOENT) {
            return proto_send_error(conn, id, "no MTD devices (/proc/mtd not found)");
        }
    ...
    LOG("mtd: read %zu bytes from /proc/mtd", len);
